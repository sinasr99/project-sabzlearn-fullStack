...
export const PUT = async req => {
    ...
    const userUpgradeResponse = await users.findOneAndUpdate({phone},{$set: {role: "Teacher"}}, {new: true})

    if (!userUpgradeResponse) {
        return Response.json("کاربری با این شماره یافت نشد", {status: 404})
    }

    return Response.json("کاربر به مدرس ارتقا یافت", {status: 200})
}