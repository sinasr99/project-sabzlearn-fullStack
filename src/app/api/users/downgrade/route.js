...
export const PUT = async req => {
    ...
    const userDownGradeResponse = await users.findOneAndUpdate({phone},{$set: {role: "User"}}, {new: true})

    if (!userDownGradeResponse) {
        return Response.json("کاربری با این شماره یافت نشد", {status: 404})
    }

    return Response.json("مدرس به کاربر ارتقا یافت", {status: 200})
}